...
@TeleOp(name = "手动阶段test", group = "Robot")
...
//            } else {
            // 基于场地中心坐标系的第三人称操控模式
            if(gamepad1.start&& gamepad1.x)headless=true;
            if(gamepad1.start&& gamepad1.y)headless=false;
            ...
            if (gamepad1.dpad_up) {
                if(driveToAprilTag(robot, 10, 2,0, -90)){
                    robot.turnToHeading(-90, 0.3);
                    PutThePixel.putThePixel(robot,IntakeArmPosition.BOARD);}
                }
            if (gamepad1.dpad_left) {
                if(driveToAprilTag(robot, 10, 1,-2, -90)) {
                    robot.turnToHeading(-90, 0.3);
                    PutThePixel.putThePixel(robot, IntakeArmPosition.BOARD);
                }
            }
            if (gamepad1.dpad_right) {
                if(driveToAprilTag(robot, 10, 3,2, -90)) {
                    robot.turnToHeading(-90, 0.3);
                    PutThePixel.putThePixel(robot, IntakeArmPosition.BOARD);
                }
            }

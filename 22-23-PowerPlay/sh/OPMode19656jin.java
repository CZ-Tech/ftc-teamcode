...
@TeleOp(name = "jin", group = "SH")
public class OPMode19656jin extends LinearOpMode {
    //region定义
...
//endregion
        ...
        while (opModeIsActive()) {
            ...
            if (status2 == Status2.one) {
                p1lx = gamepad2.left_stick_x;
                p1ly = -gamepad2.left_stick_y;
                p1rx = 0.8 * MathUtils.clamp((gamepad2.right_stick_x), -1, 1);
...
//            //region 当按下左肩键时滑轨上升，当按下右肩键时滑轨下降
            if (gamepad2.left_bumper) {
                ...
                status = status.Operation;
                if (status == Status.Ground && "ground".equals(c1label) && c1a < 300) {
                    Servo5.setPosition(0.5);
                }
                if (status == Status.Ground && "ground".equals(c2label) && c2a <300 ) {
                    Servo6.setPosition(0.49);
                }
                if (status == Status.Ground && "ground".equals(c3label) && c3a < 200 ) {
                    Servo7.setPosition(0.5);
                }
//
...
//            System.out.printf("======================label %s,%s,%s %n", c1label,c2label,c3label);
            telemetry.addData("Color1", "%d %d %d %d", c1r,c1g,c1b,c1a);
            telemetry.addData("Color2", "%d %d %d %d", c2r,c2g,c2b,c2a);
            telemetry.addData("Color3", "%d %d %d %d", c3r,c3g,c3b,c3a);
